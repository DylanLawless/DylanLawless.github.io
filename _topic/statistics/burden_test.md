...
* The multivariate analysis is robust against inclusion of noncausal variants. 
...
# Background
Genotype phenotype assocciation may be tested by:
...
* If missclassified, non-functional variants could introduce error.
...
* (This is not the same as one regression run for every single vatiant with multiple covariates, as done in GWAS sometimes).
...
The powere can be calculated as shown by the authors. 
...
* Freq above 0.01 not collaped.
* With misclassification, power is much better with CMC method compared to collasing method
	- particularly when with a high frequency of the noncausal variant.
...
## Misclassification
Two types of misclassifications are considered: 
...
* For comparison purposes, a second simulation was carried out, in which all variants were on sep- arate haplotypes.
...
* Hotelling's T2 test and the collapsing method: 
	- _well controll and slightly conservative._
...
* Reason 2: someone with more than one variant is only counted as 1 (if it is in the same allele frequency group for collapse). This may be rare in a single genebut is more likely in a protein network collapse. Usually it would be OK particularily if the second variant is in LD with the first and therefore _should_ only be counted once.
